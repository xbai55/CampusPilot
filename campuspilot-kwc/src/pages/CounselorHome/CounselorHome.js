    ...
    <>
      <section className="workbench-summary role-home-summary">
        <div className="summary-title">
          <p className="eyebrow">风险处置</p>
          <h2>识别并确认风险学生，推动预警单进入帮扶闭环。</h2>
          <p>系统会根据当前账号权限呈现对应任务、数据范围和可执行操作，界面保持统一的工作台体验。</p>
        </div>
        <div className="summary-actions">
          <Link className="primary-link" to="/warnings">处理预警单</Link>
          <Link className="text-button" to="/students">查看风险画像</Link>
        </div>
      </section>
      <section className="kpi-strip" aria-label="风险处置关键指标">
        {[["可见学生", students.length, "按角色过滤"], ["可见预警", warnings.length, "按权限范围"], ["待确认", warnings.filter((w) => w.statusKey === "todo").length, "需推进"], ["帮扶中", warnings.filter((w) => w.statusKey === "active").length, "处理中"], ["已结案", warnings.filter((w) => w.statusKey === "done").length, "已沉淀"], ["工作权限", user?.role || "辅导员", "当前账号"]].map(([label, value, note]) => (
          <div key={label} className="kpi-cell"><span>{label}</span><strong>{value}</strong><em>{note}</em></div>
        ))}
      </section>
      <section className="ops-board">
        <cp-panel eyebrow="风险处置队列" title="今天要处理什么">
          <span slot="actions" className="role-pill">当前任务</span>
          <div className="student-queue">
            {["确认待处理预警", "填写辅导员意见", "跟踪学生反馈"].map((item, i) => (
              <div key={i} className="queue-row"><span className="rank">{i + 1}</span><span className="queue-person"><strong>{item}</strong><em>识别并确认风险学生。</em></span><b>{i + 1}</b></div>
            ))}
          </div>
        </cp-panel>
