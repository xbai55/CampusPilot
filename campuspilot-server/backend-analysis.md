...
## 6. 13 个 PDF 接口的最终覆盖状态

PDF 中的 13 个 KAPI 查询接口均已接入统一 KingdeeClient。通知提醒、学习打卡、课程能力映射也已补充独立 Service、业务字段映射、REST 路由和自动化测试。Agent 的 campusContext 现在包含全部 13 类金蝶业务对象数据，不再只包含原有 4 类。